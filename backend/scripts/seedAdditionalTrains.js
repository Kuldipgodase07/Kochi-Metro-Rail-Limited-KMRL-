...
const seedAdditionalTrains = async () => {
  try {
    ...
    if (existingTrains.length > 0) {
      console.log(
        "⚠️  Some additional trains already exist. Clearing them first..."
      );
    ...
    const avgAvailability = Math.round(
      allTrainsets.reduce(
        (sum, train) => sum + train.availability_percentage,
        0
      ) / totalFleet
    );
    ...
    console.log(
      "✅ Extended maintenance trains (KMRL-008, KMRL-013, KMRL-018)"
    );
    console.log(
      "✅ Ready for premium service (KMRL-007, KMRL-009, KMRL-012, KMRL-014)"
    );
