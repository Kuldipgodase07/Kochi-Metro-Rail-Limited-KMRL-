...
const connect = async () => {
  const uri = process.env.MONGODB_URI || 'mongodb://localhost:27017/train_plan_wise';
...
const makeTrainset = (i) => {
  ...
  const availability_percentage = Math.max(0, Math.min(100, Math.round(60 + Math.random() * 40)));
...
const seed = async () => {
  ...
  console.log(`🚆 Inserted ${inserted.length} metro trainsets (KMRL-001..KMRL-040)`);

  // Update metrics snapshot
  const all = await Trainset.find();
  const tally = all.reduce((acc, t) => { acc[t.status] = (acc[t.status] || 0) + 1; return acc; }, {});
  ...
  const avgAvailability = total ? Math.round(all.reduce((s, t) => s + (t.availability_percentage || 0), 0) / total) : 0;
